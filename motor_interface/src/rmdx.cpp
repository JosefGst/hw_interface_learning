...
uint8_t RMDX::vel_cmd(int32_t speed_dps)
{
    ...
    // // only send new command if speed diff is bigger as SPEED_DIF and up to max 3 times not sent
    // if (abs(prev_vel_cmd - speed_dps) < SPEED_DIF_MIN && vel_update_count < 5)
    // {
    //     // cerr << "prev_vel_cmd: " << prev_vel_cmd << "speed_dps:" << speed_dps << endl;
    //     read_stat();
    //     vel_update_count++;
    //     // printf("counter: %d\n", vel_update_count);
    //     return 2;
    // }
    // vel_update_count = 0;
    // speed_dps = limit(speed_dps, SPEED_MIN, SPEED_MAX);
    // prev_vel_cmd = speed_dps;
